import { Button, ImageBackground, Linking, Platform, ScrollView, Text, TouchableOpacity, View } from 'react-native';
import { SafeAreaProvider, SafeAreaView } from 'react-native-safe-area-context';
import Profile from './component/profile/Profile';
import { appStyle } from './App.style';
import { useEffect, useState, useRef } from 'react';
import Wether from './component/wether/Wether';
import ToDo from './component/todo/To-DO';
import WeatherCheck from './component/WeatherCheck/Weather';
...
const navigationJson = [
    { label: "User Info", value: 'userInfo' },
    { label: "°C-°F", value: 'wether' },
    { label: "Weather", value: 'weather-check' },
...
export default function App() {
    ...
    useEffect(() => {
        registerNotification().then(token => {
            console.log(token, 'token resp')
        }).catch(e => console.log(e, 'token error'));
    ...
    async function registerNotification() {
        ...
        } else { alert("Must use physical device for push notification"); return }
        return token
    }
    return (
        <>
            {state == 'userInfo' && <SafeAreaProvider>

                <SafeAreaView style={appStyle.profile}>
                    <ScrollView scrollEnabled={true} >
                        ...
                        {userInfo.map(v => {
                            return <Profile key={v.id}{...v} openLink={openLinks} />
                        })}
                    </ScrollView>
                </SafeAreaView>
            </SafeAreaProvider>}
            {state == 'wether' &&
                <>
                    <ImageBackground source={bgImg} style={appStyle.imgBg}>
                        <SafeAreaProvider>

                            <SafeAreaView style={appStyle.profile}>
                                <Wether setBgImg={setBgImg} />
            ...
            {state == 'weather-check' && <WeatherCheck />}
            {state == 'todo' && <ToDo />}
            <View style={appStyle.navigation}>
                {navigationJson.map((v) => {
                    return <TouchableOpacity key={v.value} style={appStyle[state == v.value ? 'selectedNaveItem' : "naveItem"]} onPress={() => setState(v.value)}>
                        <Text style={appStyle['naveText']}>{v.label}</Text>
                    </TouchableOpacity>
                })}
            </View>
        </>
